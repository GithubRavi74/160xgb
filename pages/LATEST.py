...
if model is None:
    ...
    if st.button("🚀 Run AI Business Analysis", use_container_width=True):
        ...
        report_text = f"""
        ===========================================================
                iPoultry AI Guard - EXECUTIVE SUMMARY
        ===========================================================
        Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
        Batch ID: {batch_name if 'batch_name' in locals() else 'N/A'}
        Current Age: {day_number} Days
        -----------------------------------------------------------
        
        [1] BATCH STATUS & ENVIRONMENT
        - Initial Flock Size: {initial_flock:,} birds
        - Cumulative Mortality: {total_mortality:,} birds
        - Current Birds Alive: {birds_alive:,} birds
        - Temperature / RH: {temp}°C / {rh}%
        - Heat Index: {heat_index:.2f} ({'STRESS' if heat_index > 32 else 'OPTIMAL'})
        
        [2] AI WEIGHT PREDICTIONS
        - Today's Predicted Weight: {current_pred:.3f} kg
        - Target Harvest Day: Day {harvest_day}
        - Projected Harvest Weight: {projected_weight:.3f} kg
        - Performance vs. Ideal: {perf_ratio*100:.1f}%
        
        [3] FEED EFFICIENCY (FCR)
        - Total Feed Used to Date: {total_feed_to_date:,} kg
        - Estimated Current FCR: {current_fcr:.2f}
        - Projected Harvest FCR: {harvest_fcr:.2f}
        - FCR Target Variance: {harvest_fcr - 1.60:.2f}
        
        [4] FINANCIAL OUTLOOK (FORECAST)
        - Projected Total Revenue: ${revenue:,.2f}
        - Est. Production Costs: ${total_cost:,.2f}
        - PROJECTED NET PROFIT: ${profit:,.2f}
        - Estimated ROI: {(profit/total_cost)*100:.1f}%
        
        -----------------------------------------------------------
        This report is generated by the iPoultry AI Guard Model.
        Predictions are based on historical farm patterns and 
        ...
        st.download_button(
            ...
            data=report_text,
            file_name=f"iPoultry_Full_Report_Day{day_number}.txt",
            mime="text/plain",

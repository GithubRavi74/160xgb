...
if model is None:
    ...
    with t_col3:
        ...
        st.info(f"Currently Alive: {birds_alive}")
    ...
    if st.button("🚀 Run AI Business Analysis", use_container_width=True):
        ...
        # --- 8. DOWNLOAD ---
        report_text = f"iPoultry Business Report\nDay: {day_number}\nProfit: ${profit:,.2f}\nFCR: {harvest_fcr:.2f}"
        st.download_button("📄 Download Full Analysis", report_text, file_name=f"Batch_Analysis_Day{day_number}.txt", use_container_width=True)

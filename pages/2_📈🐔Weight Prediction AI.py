...
if model is None:
    ...
    if st.button("🚀 Run AI Analysis", use_container_width=True):
        ...
        # --- CALCULATE FCR ---
        # 1. Current FCR (Estimated)
        # Total feed used so far is roughly (roll_feed * day_number) 
        # but in a real scenario, you'd want the user to input 'Total Feed Consumed to Date'
        # For now, we calculate it based on your inputs:
        total_feed_to_date = st.sidebar.number_input("Total Feed Consumed to Date (kg)", value=feed_today * day_number)
        
        # FCR = Total Feed / (Current Avg Weight * Birds Alive)
        ...
        # 2. Projected Harvest FCR
        # We estimate the total feed they will eat by harvest day
